...
for index, item in enumerate(X_full):
    if phoneme_id[index] == p_id:
        if X_phoneme.size != 0:
            X_phoneme = np.append(X_phoneme, [item], axis = 0)
        else:
            X_phoneme = np.array([item])
mf1 = np.mean(X_phoneme[:,0])
mf2 = np.mean(X_phoneme[:,1])
mf1f2 = np.mean(X_phoneme[:,1])
stdf1 = np.std(X_phoneme[:,0])
stdf2 = np.std(X_phoneme[:,1])
stdf1f2 = np.std(X_phoneme[:,2])
# X_phoneme[:,0] = (X_phoneme[:,0]-mf1)/stdf1
# X_phoneme[:,1] = (X_phoneme[:,1]-mf2)/stdf2
# X_phoneme[:,2] = (X_phoneme[:,2]-mf1f2)/stdf1f2
